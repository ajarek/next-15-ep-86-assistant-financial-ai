...
const FinancialGoals = () => {
  ...
  const overallProgress =
    (items.reduce((acc, goal) => goal.currentAmount, 0) /
      items.reduce((acc, goal) => goal.targetAmount, 0)) *
    100
  ...
  return (
    <div className='min-h-screen w-full  flex flex-col gap-4  rounded-xl shadow-md overflow-hidden px-6'>
      ...
      {items.length !== 0 ? (
            ...
            {items
              ...
              .map((goal) => (
                ...
                >
                  ...
                  <Button
                    onClick={() => removeItemFromGoals(goal.id)}
                    size={'icon'}
                    variant={'outline'}
                    className='bg-transparent mx-4  hover:text-xl transition-all duration-300  '
                  >
                    ❌
                  </Button>
                </div>
        ...
        <div className='flex flex-col gap-4 text-xl text-red-500'>
          <h1>Brak celów finansowych !</h1>
          <Link
            href={'/dashboard/goals/add'}
            className='w-full text-center  px-4 py-2 bg-primary text-white rounded-lg my-6'
          >
